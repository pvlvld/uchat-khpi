...
char *encrypt(char *public_key_str, const char *data) {
    ...
    // Определение длины зашифрованного сообщения
    ...
    // Освобождение ключа и контекста после шифрования
    EVP_PKEY_free(pkey);
    EVP_PKEY_CTX_free(ctx);
    ...
    BIO_push(b64, mem);
    BIO_set_flags(b64, BIO_FLAGS_BASE64_NO_NL); // Без перехода на новую строку в Base64
    ...
    // Копируем данные в строку с завершающим нулем
