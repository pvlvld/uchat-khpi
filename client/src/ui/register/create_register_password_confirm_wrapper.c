...
static void on_toggle_password_visibility(GtkToggleButton *toggle_button, gpointer data) {
    ...
    gtk_entry_set_visibility(GTK_ENTRY(form_data->password_confirm_entry), is_visible);

    if (is_visible) {
        gtk_image_set_from_file(GTK_IMAGE(form_data->toggle_confirm_image), "resources/images/static/cross.svg");
    } else {
        gtk_image_set_from_file(GTK_IMAGE(form_data->toggle_confirm_image), "resources/images/static/eye.svg");
    }
}
...
GtkWidget *create_register_password_confirm_wrapper(void) {
    ...
    vendor.pages.register_page.password_confirm_wrapper = register_password_wrapper;
    ...
    form_data->password_confirm_entry = password_entry;

    GtkWidget *password_placeholder = gtk_label_new("Confirm password");
    ...
    form_data->toggle_confirm_image = toggle_image;
