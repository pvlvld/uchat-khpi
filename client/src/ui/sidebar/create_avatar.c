...
GtkWidget *chatblock_create_avatar(t_chat_info *chat_info) {
    ...
    set_classname_and_id(fixed_container, "chatblock__avatar");

    GtkWidget *avatar = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
    set_classname_and_id(avatar, "chatblock__avatar_image");

    gchar avatar_class_str[64];
    g_snprintf(avatar_class_str, sizeof(avatar_class_str), "chatblock__avatar_class-%d", chat_info->id);
    ...
    set_classname_and_id(group_type, "chatblock__avatar_group-type");
    ...
    if (chat_info->unreaded_messages != 0) {
        ...
        set_classname_and_id(number_of_messages, "chatblock__avatar_number-of-messages");
